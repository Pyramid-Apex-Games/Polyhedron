//////////////////////////////////////////////////////////////////////////////
//
// Create cube 2 font glyph pages.
//
// Authors:
//
//   Quinton "Quin" Reeves and Lee "eihrul" Salzman:
//       Original implementation, including (but not limited to) the PNG export
//       code, packing algorithm, code page definition, and config file and
//       tex file creation.
//
//   Tim Thomas:
//       - Generally refactored the code, functionalizing, adding comments,
//         variable-length allocated string support, etc.
//       - Extended Lee Salzman's PNG export code to support color PNGs, with
//         and without alpha.
//       - Added additional error checking and Free Type error reporting.
//       - Added alternate font glyph packing algorithm.
//       - Added option parser.
//       - Added the following features (controled by options):
//         - Glyph mapping using possibly multiple alternate fonts
//         - Fatal error on missing glyphs
//         - Negative inborder and outborder values
//         - Foreground and background color settings
//         - Digit-only output
//         - Forced fixed width character (metrics) output
//         - Config file font name setting
//         - Help and version display
//         - Noexec mode
//
//   Martin Erik Werner:
//       Wrote the cube2font manpage.  Some of the manpage text was incorporated
//       by Tim Thomas into the option help.
//
// License:
//
//   As with all Sauerbraten source code, this is covered by the ZLIB license
//   (http://www.opensource.org/licenses/zlib-license.php).  Please see the
//   file src/readme_source.txt in the Sauerbraten source distribution for
//   full details.
//
// Versions:
//
//   0.0:
//      Original code by Quinton Reeves and Lee Salzman.
//
//   1.0:
//      First version by Tim Thomas
//
//////////////////////////////////////////////////////////////////////////////
...
#define APP_NAME "cube2font-enhanced"
#define APP_VERSION "1.0"
#define APP_COPYRIGHT "\
Copyright (C) 2012 Quinton Reeves, Lee Salzman, Martin Erik Werner, Tim Thomas \n\
...
#define CODE_PAGE_SIZE (sizeof(Cube2UnicharList)/sizeof(Cube2UnicharList[0]))
...
const uchar Cube2CharTypeList[CODE_PAGE_SIZE] =
{
    CUBECTYPE(CT_SPACE,
              CT_PRINT,
              CT_PRINT | CT_DIGIT,
...
IsCubePrint(uchar c)
{
    return Cube2CharTypeList[c] & CT_PRINT;
}
...
IsCubeSpace(uchar c)
{
    return Cube2CharTypeList[c] & CT_SPACE;
}

int
IsCubeAlpha(uchar c)
{
    return Cube2CharTypeList[c] & CT_ALPHA;
}
#endif
...
IsCubeNum(uchar c)
{
    return Cube2CharTypeList[c] & CT_DIGIT;
}

#if 0
int
IsCubeAlphaNum(uchar c)
{
    return Cube2CharTypeList[c] & (CT_ALPHA | CT_DIGIT);
}

int
IsCubeLower(uchar c)
{
    return Cube2CharTypeList[c] & CT_LOWER;
}

int
IsCubeUpper(uchar c)
{
    return Cube2CharTypeList[c] & CT_UPPER;
}
#endif
    ...
    FontCharP chars, FontCharP *order, size_t codePageSize)
{
    ...
    if ((error = FT_Stroker_New(type_library, &stroker_out_border)) ||
    ...
    for (code_page_idx = 0; code_page_idx < codePageSize; code_page_idx++)
    {
        ...
        if (!IsCubePrint(code_page_idx))
        {
            continue;
        }

        if (prog_opts_p->digits && !IsCubeNum(code_page_idx))
        {
             continue;
        }

        dest_char->code = code_page_idx;
        dest_char->uni = Cube2Uni(code_page_idx);
